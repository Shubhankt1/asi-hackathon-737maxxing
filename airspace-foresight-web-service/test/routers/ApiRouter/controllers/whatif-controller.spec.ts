...
describe("WhatifControllerController", () => {
  let app: any;
  ...
  it("GET /api/whatif returns 200", async () => {
    const res = await request(app).get("/api/whatif");
    assertOk(res);
  });

  it("GET /api/whatif returns 500 when an error occurs", async () => {
    ...
    // });
    // const res = await request(errApp).get("/api/whatif");
