...
describe("PositionsControllerController", () => {
  let app: any;
  ...
  it("GET /api/positions returns 200", async () => {
    const res = await request(app).get("/api/positions");
    assertOk(res);
  });

  it("GET /api/positions returns 500 when an error occurs", async () => {
    ...
    // });
    // const res = await request(errApp).get("/api/positions");
