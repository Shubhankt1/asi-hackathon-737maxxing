...
describe("DemandControllerController", () => {
  let app: any;
  ...
  it("GET /api/demand returns 200", async () => {
    const res = await request(app).get("/api/demand");
    assertOk(res);
  });

  it("GET /api/demand returns 500 when an error occurs", async () => {
    ...
    // });
    // const res = await request(errApp).get("/api/demand");
