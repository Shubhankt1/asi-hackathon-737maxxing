...
describe("SectorsControllerController", () => {
  let app: any;
  ...
  it("GET /api/sectors returns 200", async () => {
    const res = await request(app).get("/api/sectors");
    assertOk(res);
  });

  it("GET /api/sectors returns 500 when an error occurs", async () => {
    ...
    // });
    // const res = await request(errApp).get("/api/sectors");
