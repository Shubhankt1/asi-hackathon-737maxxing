...
describe("ConflictsControllerController", () => {
  let app: any;
  ...
  it("GET /api/conflicts returns 200", async () => {
    const res = await request(app).get("/api/conflicts");
    assertOk(res);
  });

  it("GET /api/conflicts returns 500 when an error occurs", async () => {
    ...
    // });
    // const res = await request(errApp).get("/api/conflicts");
