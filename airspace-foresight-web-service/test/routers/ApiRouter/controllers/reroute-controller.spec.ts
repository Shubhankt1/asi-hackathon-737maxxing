...
describe("RerouteControllerController", () => {
  let app: any;
  ...
  it("GET /api/reroute returns 200", async () => {
    const res = await request(app).get("/api/reroute");
    assertOk(res);
  });

  it("GET /api/reroute returns 500 when an error occurs", async () => {
    ...
    // });
    // const res = await request(errApp).get("/api/reroute");
