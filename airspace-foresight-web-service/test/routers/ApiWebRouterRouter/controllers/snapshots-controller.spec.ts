...
describe("SnapshotsControllerController", () => {
  let app: any;
  ...
  it("GET /api-web-router/snapshots returns 200", async () => {
    const res = await request(app).get("/api-web-router/snapshots");
    assertOk(res);
  });

  it("GET /api-web-router/snapshots returns 500 when an error occurs", async () => {
    ...
    // });
    // const res = await request(errApp).get("/api-web-router/snapshots");
